...
contract TokenFarm is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        uint8 _rateOfRewards,
        uint256 _rewardDurationInSecond,
        uint256 _totalDurationContract,
    ...
    event stakeEvent(address staker, uint256 amount);

    event unstakeEvent(address staker, uint256 amount);

    event forcewithdrawEvent(address staker, uint256 amount);

    event rewardEvent(address staker, uint256 amount);
    ...
    function ForceWithdraw() public {
        ...
        uint256 remainBalance = InitialBalance[stakerAddress] -
            TotalRewardsUpToNow[stakerAddress];
        ...
        if (success == true) {
            ...
            adminLogs.push(log);
            TransactionHistory memory transactionHist = TransactionHistory(
                "force withdraw",
                block.timestamp,
                remainBalance
            );
    ...
    function calcualteRewards() internal onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            if (
                (LastRewardTimeStamp[stakerAddress] + rewardDurationInSecond) <
                block.timestamp
            ) {
    ...
    function IsStaker() public view returns (bool result) {
        bool result = false;
        for (uint256 i = 0; i < stakers.length; i++) {
            if (stakers[i] == msg.sender) {
                bool result = true;
    ...
    function isStakerByAddress(address _stakerAddress)
        public
        view
        returns (bool result)
    {
    ...
    function calculateTotalReward() public view returns (uint256 totalReward) {
        if (IsStaker() == true) {
            ...
            uint256 totalRewardStaker = countRewards *
                ((InitialBalance[msg.sender] * rateOfReward) / 100);
    ...
    function stakeTokens(uint256 _amount) public {
        ...
        require(
            _amount <= maximumDeposit,
            "amount must be little than maximum deposit"
        );
        require(
            dappToken.balanceOf(msg.sender) > _amount,
            "not enough token in your wallet!"
        );
        bool success = dappToken.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function unstakeTokens() public onlyOwner {
        ...
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            if (
                stakingTimeStamp[stakerAddress] <
                block.timestamp - totalDurationContract
            ) {
                ...
                if (success == true) {
                    ...
                    adminLogs.push(log);
                    TransactionHistory
                        memory transactionHist = TransactionHistory(
                            "unstaking",
                            block.timestamp,
                            balance
                        );
    ...
    {
        uint256 transCount = TransactionHistories[msg.sender].length;
        TransactionHistory[] memory lTrans = new TransactionHistory[](
            transCount
        );
        for (uint256 i = 0; i < transCount; i++) {
            TransactionHistory storage thisTrans = TransactionHistories[
                msg.sender
            ][i];
            lTrans[i] = thisTrans;
        }
    ...
    function filterAdminlog(
        string memory typeOfTransaction,
        uint256 startDate,
        uint256 endDate
    ) public view onlyOwner returns (AdminLog[] memory) {
        uint256 count = getFilteredCount(typeOfTransaction, startDate, endDate);
        ...
        for (uint256 i = 0; i < adminLogs.length; i++) {
            ...
            if (
                keccak256(bytes(thisTrans.transactionType)) ==
                keccak256(bytes(typeOfTransaction)) &&
                thisTrans.transactionTimeStamp > startDate &&
                thisTrans.transactionTimeStamp < endDate
            ) {
